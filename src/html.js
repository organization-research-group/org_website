...
function renderEntity(val, key) {
  ...
  return h("div", [
    ...
    h(
      "ul",
      val.map(({ fragment, roles, label }) =>
        h("li", [
          h("a", { href: `directory.html#${fragment}` }, label),
          roles.size === 0
            ? null
            : h("span.entity-role", [" (", [...roles].join(", "), ")"]),
        ]),
      ),
    ),
  ]);
...
function renderLinks(links) {
  ...
  return h("div", [
    ...
    h(
      "ul",
      links.map((href) => h("li", [h("a", { href }, href)])),
    ),
  ]);
}
  ...
  return async (meeting) => {
    const renderScheduleItem = async ($scheduleItem) => {
      ...
      if (bibItem) {
        ...
        const description = getFirstObjectLiteral(
          store,
          $scheduleItem,
          "dc:description",
        );
      ...
      return new Promise((resolve, reject) =>
        toHyperscript(ret.trim(), (err, tree) => {
          if (err) {
            reject(err);
          }
    ...
    const scheduleHTML = await Promise.all(
      meeting.schedule.map(renderScheduleItem),
    );
...
async function renderMain(grist) {
  ...
  return renderPage(
    h("div", [
      h("section#about", [
        h("p", [
          "ORG is a reading group at UNC Chapel Hill that meets Fridays at 10:00am ET ",
      ...
      h("section#current", [h("h2", "Next meeting"), meetings[0]]),
...
function renderDirectory(grist) {
  ...
  const htmlByType = R.pipe(
    ...
    R.map(
      R.map(({ uri, label, weeks = [], homepage }) =>
        h("div#" + uri.split("#")[1], [
          h("h3", label),
          ...
          h(
            "ul",
            weeks.map((week) =>
              h("li", [h("a", { href: week }, week.split(":")[1])]),
            ),
          ),
        ]),
      ),
  ...
  return renderPage(
    h("section#index", [
      h("div", [h("h2", "People"), htmlByType.People]),

      h("div", [h("h2", "Journals"), htmlByType.Journals]),
...
async function renderArchive(grist) {
  const meetings = await Promise.all(grist.meetings.map(renderMeeting(grist)));

  return renderPage(h("section", meetings));
}
      ...
      h(".content-wrapper", [
        h("header", [
          h(
            ...
            [].concat(
              "The ",
              "Organization Research Group".split("").map((letter) => {
                let classNames = "";
        ...
        h("footer", [
          h("div", ["Last updated ", lastUpdated]),
          ...
          h("div", [
            ...
            h(
              "a",
              {
                href: "https://github.com/organization-research-group/org_website",
              },
              "Source",
            ),
          ]),
